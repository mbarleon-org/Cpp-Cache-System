// Fragmented cache behavior tests.
...
int main()
{
    ...
    using Cache = cache::Fragmented<K, V, cache::strategy::LRU<K, V>, // Strategy
    ...
    // --- Callback invalidation, including a fragment created after registration ---
    {
        ...
        cache.put(0, -100); // create shard 0 before registering the callback
        cache.invalidateIf([&callback_calls](const K& key, const V& value) {
            ++callback_calls;
        ...
        check_true("cleared predicate does not apply to a subsequently created shard", cache.get(2, out));
        check_eq("cleared fragmented predicate is not invoked", callback_calls, 3);
        cache.clear();
    }
